...
# InceptionV3 모델 구축 및 수정
base_model = InceptionV3(weights='imagenet', include_top=False, input_shape=(299, 299, 3))
x = GlobalAveragePooling2D()(base_model.output)
x = Dense(1024, activation='relu', kernel_regularizer=l2(0.001))(x)  # l2 정규화
x = Dropout(0.5)(x)
...
model_checkpoint = ModelCheckpoint(
    filepath=os.path.join(models_dir, 'justMeatInception_BM.keras'),
    save_best_only=True,
    monitor='val_loss',
    mode='min'
)
...
# 학습이 완료된 후 모델 저장
model.save(os.path.join(models_dir, 'justMeatInception.keras'))

with open(os.path.join(models_dir, 'justMeatInception.pkl'), 'wb') as file:
